...
*/
	int n,q;	
	cin>>n>>q;//read(n,q);
	for(int i=1;i<=n;++i)cin>>b[i];//read(b[i]);
	for(int i=1;i<=n;++i){
		cin>>a[i];//read(a[i]);
		a[i]-=b[i];
		s[i]=a[i]+s[i-1];
	...
	for(;q;--q){
		...
		if(s[l-1]==s[r]){
			int h=lg[r-l+1];
			int xmn=min(mn[h][l],mn[h][r-(1<<h)+1]);
			int xmx=max(mx[h][l],mx[h][r-(1<<h)+1]);
			write(xmn<s[l-1]?0:xmx-s[l-1],'\n');
		}
		else write("-1\n");
	}
	return 0;
