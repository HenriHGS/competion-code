...
void dfs(int x){
    if(x > n){
        mcf_graph<int,int> mc(40);
        int S = 0, T = n + n + 1;
        rep(i,1,n) r[i] = a[i] - l[i];
        int lsum = 0, rsum = 0;
        rep(i,1,n) lsum += l[i], rsum += r[i];
        rep(i,1,n){
            mc.add_edge(S, i, l[i], 0);
            mc.add_edge(i + n, T, r[i], 0);
            rep(j,1,n)mc.add_edge(i, n + j, INF, c[i][j]);
        }
        pair<int,int> res = mc.flow(S,T);
        if(res.first == lsum && rsum == lsum){
            if(ans == 0) ans = res.second;
            else ans = min(ans,res.second);
        }
        return ;
    }
    l[x] = a[x]/2;
